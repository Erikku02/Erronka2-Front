...
new Vue({
    ...
    computed: {
        ...
        itemsFiltradosPaginados() {
            let itemsFiltrados = this.listaKategoria;
        ...
        // Paginar
        cantidadPorPaginas() {
            return Math.ceil(this.listaKategoria.length / this.itemsPorPagina)
        },
    },
    methods: {
        paginaAnterior() {
        ...
        selectAllKategoriak() {
            if (this.selectAllKatego) {
                // Si el checkbox de seleccionar todo está marcado,
                // se añaden todos los IDs a arrayId
                this.arrayId = this.listaKategoria.map(kategoria => kategoria.id);
            } else {
